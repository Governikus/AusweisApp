			...
			Text {
				...
				text: {
					if (!pinField.confirmedInput) {
						qsTr("The entered PIN does not match the new PIN. Please correct your input.")
					} else if (!!numberModel.inputError) {
						numberModel.inputError
					} else {
						baseItem.state === "CAN" ? qsTr("You have entered the wrong PIN twice. For a third attempt, you have to enter your six-digit card access number first. You can find your card access number on the front of your ID card.") :
						...
						baseItem.state === "PIN_NEW" ? qsTr("Please enter a new arbitrary 6-digit PIN.") :
						baseItem.state === "PIN_NEW_AGAIN" ? qsTr("Please confirm your new PIN by re-entering your personal PIN.") :
						baseItem.state === "PIN" ? qsTr("Please enter your personal PIN.") :
						/*"PIN_OR_TRANSPORT_PIN"*/ qsTr("Please enter your old PIN or your initial transport PIN first.")
					}
				}
			}
			Item {/*spacer*/ Layout.fillWidth: true; height: parent.height}
