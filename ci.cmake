...
macro(PARSE_FROM_NAME)
	...
	if(NAME)
		...
		if((NAME MATCHES "_Review_" OR DEFINED ENV{REVIEWBOARD_REVIEW_REQUEST} OR DEFINED ENV{PATCH_URL}) AND NOT DEFINED REVIEW)
			set(REVIEW ON)
		...
		if(NOT SCRIPT)
			set(CMAKE_CI_DIR "${CMAKE_SOURCE_DIR}/ci/cmake")
			fetch_script(SCRIPT CMAKE_MODULE_PATH "${CMAKE_CI_DIR}" "${NAMES}")
			list(APPEND CMAKE_MODULE_PATH ${CMAKE_CI_DIR})
		endif()
	endif()
...
function(RUN)
	...
	if(REVIEW)
		if(NOT PATCHED AND DEFINED ENV{GITLAB_CI})
			include(Files)
			set(patch.diff "${CMAKE_BINARY_DIR}/patch.diff")
			if(DEFINED ENV{PATCH_URL})
				step(uv pip install python-hglib)
				handle_http("${patch.diff}" URLS $ENV{PATCH_URL})
			elseif(DEFINED ENV{REVIEWBOARD_REVIEW_REQUEST})
				step(uv pip install rbtools python-hglib)
				step(rbt patch --write ${patch.diff} --server $ENV{REVIEWBOARD_SERVER} --diff-revision $ENV{REVIEWBOARD_DIFF_REVISION} $ENV{REVIEWBOARD_REVIEW_REQUEST})
			elseif(EXISTS "${patch.diff}")
				message(STATUS "Use existing patch: ${patch.diff}")
			else()
