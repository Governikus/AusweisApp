 ...
 *
 * \copyright Copyright (c) 2014-2017 Governikus GmbH & Co. KG
 */
	...
	: public QObject
{
	...
	private Q_SLOTS:
		...
		void testPurposeOf_2014_07_03_cvcDescription()
		{
			...
			QSharedPointer<CertificateDescription> certDescr = CertificateDescription::fromHex(TestFileHelper::readFile(filename).toHex());
			QVERIFY(certDescr);
		...
		void testGetDescTestAutentServer()
		{
			...
			QSharedPointer<CertificateDescription> certDescr = CertificateDescription::fromHex(hexValue);
			QVERIFY(certDescr);
