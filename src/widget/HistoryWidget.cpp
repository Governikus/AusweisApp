	...
	, mHistoryDetailWidget(nullptr)
{
	...
	connect(&AppSettings::getInstance().getHistorySettings(), &HistorySettings::fireHistoryInfosChanged, this, &HistoryWidget::updateTable);
	connect(&AppSettings::getInstance().getHistorySettings(), &HistorySettings::fireEnabledChanged, this, [this](bool pValue){
...
{
	const auto& items = AppSettings::getInstance().getHistorySettings().getHistoryInfos();
	...
	for (const HistoryInfo& info : items)
	{
		...
		QLabel* requestedDataLabel = new QLabel(info.getRequestedData().trimmed());
		...
		centralWidget->setProperty("requestedData", info.getRequestedData());
...
#else
			tr("PDF Documents") + QStringLiteral(" (*.pdf)"), nullptr, QFileDialog::DontUseNativeDialog);
#endif
