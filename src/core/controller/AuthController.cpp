...
{
	auto sProcessing = addAndConnectState<StateProcessing>();
	mStateMachine.setInitialState(sProcessing);
	auto sParseTcTokenUrl = addAndConnectState<StateParseTcTokenUrl>();
	...
	auto sRedirectBrowser = addAndConnectState<StateRedirectBrowser>();
	auto sCleanUpReaderManager = addAndConnectState<StateCleanUpReaderManager>();
	auto sFinal = addAndConnectState<FinalState>();

	sProcessing->addTransition(sProcessing, &AbstractState::fireSuccess, sParseTcTokenUrl);
	sProcessing->addTransition(sProcessing, &AbstractState::fireError, sCleanUpReaderManager);
	sProcessing->addTransition(sProcessing, &AbstractState::fireCancel, sCleanUpReaderManager);

	sParseTcTokenUrl->addTransition(sParseTcTokenUrl, &AbstractState::fireSuccess, sGetTcToken);
	sParseTcTokenUrl->addTransition(sParseTcTokenUrl, &AbstractState::fireError, sCleanUpReaderManager);
	sParseTcTokenUrl->addTransition(sParseTcTokenUrl, &AbstractState::fireCancel, sCleanUpReaderManager);
	...
	sCleanUpReaderManager->addTransition(sCleanUpReaderManager, &AbstractState::fireSuccess, sCheckRefreshAddress);
	sCleanUpReaderManager->addTransition(sCleanUpReaderManager, &AbstractState::fireError, sCheckRefreshAddress);
	sCleanUpReaderManager->addTransition(sCleanUpReaderManager, &AbstractState::fireCancel, sCheckRefreshAddress);

	sCheckRefreshAddress->addTransition(sCheckRefreshAddress, &AbstractState::fireSuccess, sWriteHistory);
	sCheckRefreshAddress->addTransition(sCheckRefreshAddress, &AbstractState::fireError, sRedirectBrowser);
	sCheckRefreshAddress->addTransition(sCheckRefreshAddress, &AbstractState::fireCancel, sRedirectBrowser);

	sWriteHistory->addTransition(sWriteHistory, &AbstractState::fireSuccess, sRedirectBrowser);
	sWriteHistory->addTransition(sWriteHistory, &AbstractState::fireError, sRedirectBrowser);
	sWriteHistory->addTransition(sWriteHistory, &AbstractState::fireCancel, sRedirectBrowser);

	sRedirectBrowser->addTransition(sRedirectBrowser, &AbstractState::fireSuccess, sFinal);
	sRedirectBrowser->addTransition(sRedirectBrowser, &AbstractState::fireError, sFinal);
	sRedirectBrowser->addTransition(sRedirectBrowser, &AbstractState::fireCancel, sFinal);
}
