...
void WebSocketChannel::close()
{
	if (mConnection)
	{
		...
		{
			qCWarning(remote_device) << "Tried to close web socket from the wrong thread: skipping";
		}
	}
}
...
{
	qCDebug(remote_device) << "No pong received from remote for" << (PING_PONG_TIMEOUT_MS / 1000) << "seconds, closing socket";
	close();
}
