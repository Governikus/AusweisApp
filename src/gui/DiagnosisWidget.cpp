	...
	, mTimestampItem(nullptr)
{
	...
	mAppVersionField.reset(insertField(insertCursor, QCoreApplication::applicationName() + " - Version"));
	...
	mAppVersionTreeItem->setText(0, QCoreApplication::applicationName() + " - Version");
...
void DiagnosisWidget::onReaderInfosChanged()
{
	...
	for (const ReaderInfo& info : infos)
	{
		...
		{
			case CardType::NONE:
				cardType = tr("not inserted", "Karte");
				break;

			case CardType::UNKNOWN:
				cardType = tr("unknown type", "Karte");
				break;

			case CardType::EID_CARD:
				cardType = tr("ID card (PA/eAT)");
				break;
		...
		// retry counter
		if (info.getCardType() == CardType::EID_CARD)
		{
			insertCursor.insertText(QStringLiteral("\n"), mPlainTextFormat);
...
{
	QString timestamp = mContext->getTimestamp().toString(tr("MMMM dd yyyy, hh:mm:ss AP"));
