	...
	, mSelectedReader(pSelectedReader)
{
	...
	setWindowTitle(QCoreApplication::applicationName() + " - " + tr("On screen password"));

	setModal(true);

	connect(&ReaderManager::getInstance(), &ReaderManager::fireCardRemoved, this, &RandomPinDialog::onCardRemoved);
	connect(&ReaderManager::getInstance(), &ReaderManager::fireCardInserted, this, &RandomPinDialog::onCardInserted);
